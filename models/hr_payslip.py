...
class HrPayslip(models.Model):
    ...
    struct_id = fields.Many2one(comodel_name='hr.payroll.structure',
                                ...
                                help="Define las reglas que deben aplicarse a este recibo de nómina, "
            "de acuerdo con el contrato seleccionado. "
            "Si el campo de contrato se deja vacío, este campo deja de ser obligatorio, "
            "y se aplicarán todas las reglas establecidas en las estructuras "
            "de los contratos válidos del empleado para el período elegido.")
    name = fields.Char(string='Nombre del recibo', help="Nombre del recibo de nomina")
    number = fields.Char(string='Referencia', copy=False,
                         help="Referencia del recibo de nomina", )
    employee_id = fields.Many2one(comodel_name='hr.employee', string='Empleado',
                                  required=True,
                                  help="Empleado para este recibo de nomina")
    date_from = fields.Date(string='Desde', required=True,
                            help="Fecha de inicio del perido del recibo de nomina",
                            default=lambda self: fields.Date.to_string(
                                date.today().replace(day=1)))
    date_to = fields.Date(string='Hasta', required=True,
                          help="Fecha de finalizacion del perido del recibo de nomina",
                          default=lambda self: fields.Date.to_string(
    ...
    state = fields.Selection(selection=[
        ...
        help=( "* Cuando se crea el recibo de nómina, el estado es 'Borrador'.\n"
            "* Si el recibo está en verificación, el estado es 'En espera'.\n"
            "* Si el recibo se confirma, el estado pasa a 'Confirmado'.\n"
            "* Si el usuario cancela el recibo, el estado es 'Rechazado'."))
    line_ids = fields.One2many('hr.payslip.line',
                               'slip_id',
                               string='Lineas del recibo',
                               help="lineas asociadas a este recibo de nomina")
    ...
    worked_days_line_ids = fields.One2many('hr.payslip.worked.days',
                                           ...
                                           copy=True,
                                           help="Dias trabajados relacionados con este recibo de nomina")
    input_line_ids = fields.One2many('hr.payslip.input',
                                     'payslip_id',
                                     string='Entradas de Nómina',
                                     help="Entradas adicionales asociadas al recibo de nómina")
    paid = fields.Boolean(string='¿Orden de Pago Generada?',
                          copy=False, help="Indica si se ha generado una orden de pago para este recibo")
    note = fields.Text(string='Nota interna', help="Descripcion o comentarios")
    contract_id = fields.Many2one('hr.contract', string='Contracto',
                                  help="Contrato del empleado asociado a este recibo de nomina")
    details_by_salary_rule_category_ids = fields.One2many(
        ...
        string='Detalles por Categoría de Regla Salarial', help="Detalles agrupados por categorías de reglas salariales")
    credit_note = fields.Boolean(string='Nota de credito',
                                 help="Indica que este recibo de nomina corresponde a un reembolso u otro ajuste")
    payslip_run_id = fields.Many2one('hr.payslip.run',
                                     string='Lote de Recibos',
                                     copy=False, help="Lote de recibos de nomina")
    payslip_count = fields.Integer(compute='_compute_payslip_count',
                                   string="Detalle de calculo del recibo",
                                   help="Cantidad o detalle de los recibos calculados")
