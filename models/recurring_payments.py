...
class RecurringPayments(models.Model):
    ...
    name = fields.Char(string='Name', tranlate=True)
    debit_account = fields.Many2one('account.account',
                                    'Debit Account',
                                    required=True,
                                    domain="['|', ('company_id', '=', False), "
                                           "('company_id', '=', company_id)]", tranlate=True)
    ...
    journal_id = fields.Many2one('account.journal',
                                 'Journal', required=True, tranlate=True)
    analytic_account_id = fields.Many2one('account.analytic.account',
                                          'Analytic Account', tranlate=True)
    date = fields.Date('Starting Date', required=True,
                       default=date.today(), tranlate=True)
    next_date = fields.Date('Next Schedule',
                            compute=_get_next_schedule,
                            readonly=True, copy=False, tranlate=True)
    recurring_period = fields.Selection(selection=[('days', 'Days'),
                                                   ('weeks', 'Weeks'),
                                                   ('months', 'Months'),
                                                   ('years', 'Years')],
                                        store=True, required=True, tranlate=True)
    amount = fields.Float('Amount', tranlate=True)
    description = fields.Text('Description', tranlate=True)
    state = fields.Selection(selection=[('draft', 'Draft'),
                                        ('running', 'Running')],
                             default='draft', string='Status', tranlate=True)
    journal_state = fields.Selection(selection=[('draft', 'Unposted'),
                                                ('posted', 'Posted')],
                                     required=True, default='draft',
                                     string='Generate Journal As', tranlate=True)
    recurring_interval = fields.Integer('Recurring Interval', default=1, tranlate=True)
    partner_id = fields.Many2one('res.partner', 'Partner', tranlate=True)
    pay_time = fields.Selection(selection=[('pay_now', 'Pay Directly'),
                                           ('pay_later', 'Pay Later')],
                                store=True, required=True, tranlate=True)
    company_id = fields.Many2one('res.company',
                                 default=lambda l: l.env.company.id, tranlate=True)
    recurring_lines = fields.One2many(
        'account.recurring.entries.line', 'tmpl_id', tranlate=True)
...
class GetAllRecurringEntries(models.TransientModel):
    ...
    date = fields.Date('Date', tranlate=True)
    template_name = fields.Char('Name', tranlate=True)
    amount = fields.Float('Amount', tranlate=True)
