    ...
    try:
        for item in data.get('DesgloseVxH', []):
            ...
            for prod in item.get('Productos', []):
                third_party_id = str(prod.get('IdTerceros', 0))
                print("THIRD PARTY ID RECIBIDO:", third_party_id)
        ...
        raise SaleSummaryCreationError(
            400,
            f'Error durante lectura de datos de desglose vxh hhhkhjk: {str(e)}'
        )
