...
class FinancialReport(models.TransientModel):
    ...
    def view_report_pdf(self):
        ...
        # checking view type
        print("data end: ", data)
        ctx = self.env['report.base_accounting_kit.report_financial']._get_report_values(data=data)
        return self.env.ref('contabilidad_kuale.financial_report_pdf').report_action(self,data=ctx)
    ...
    def get_account_lines(self, data):
        ...
        for report in child_reports:
            r_name = re.sub(r'[^0-9a-zA-Z]+', '', report.name)
            p_name = False
            if report.parent_id:
                p_name = re.sub(r'[^0-9a-zA-Z]+', '', report.parent_id.name) + str(report.parent_id.id)
            ...
            if data['debit_credit']:
                report_vals['debit'] = res[report.id]['debit']
                report_vals['credit'] = res[report.id]['credit']
            if data['enable_filter']:
                report_vals['balance_cmp'] = res[report.id].get('comp_bal', 0.0) * int(report.sign)

            lines.append(report_vals)

            if report.display_detail == 'no_detail':
                continue

            if not res[report.id].get('account'):
                continue

            # 1. Obtener cuentas organizadas por grupo
            accounts_by_group = {}
            for account_id, values in res[report.id]['account'].items():
                account = Account.browse(account_id)
                group = account.group_id or self.env['account.group']
                accounts_by_group.setdefault(group.id or 0, {
                    'group': group,
                    'accounts': [],
                })['accounts'].append((account, values))

            # 2. Iterar por grupo (ordenados por nombre o id)
            for group_id in sorted(accounts_by_group):
                group_data = accounts_by_group[group_id]
                group = group_data['group']
                group_uid = f"{r_name}_group_{group.id or 'nogroup'}"
                group_vals = {
                    'name': group.name or 'Sin grupo',
                    'id': group_uid,
                    'parent': r_name + str(report.id),
                    'level': (int(report.style_overwrite) if report.style_overwrite else report.level) + 1,
                    'type': 'group',
                    'balance': 0.0,
                }

                if data['debit_credit']:
                    group_vals['debit'] = 0.0
                    group_vals['credit'] = 0.0
                if data['enable_filter']:
                    group_vals['balance_cmp'] = 0.0

                account_lines = []

                for account, value in sorted(group_data['accounts'], key=lambda x: x[0].code):
                    acc_vals = {
                        'account': account.id,
                        'a_id': account.code + re.sub('[^0-9a-zA-Z]+', 'acnt', account.name) + str(account.id),
                        'name': f"{account.code} - {account.name}",
                        'balance': value['balance'] * int(report.sign),
                        'type': 'account',
                        'parent': group_uid,
                        'level': group_vals['level'] + 1,
                        'account_type': account.account_type,
                    }
                    if data['debit_credit']:
                        acc_vals['debit'] = value['debit']
                        acc_vals['credit'] = value['credit']
                        group_vals['debit'] += value['debit']
                        group_vals['credit'] += value['credit']
                    if data['enable_filter']:
                        acc_vals['balance_cmp'] = value.get('comp_bal', 0.0) * int(report.sign)
                        group_vals['balance_cmp'] += acc_vals['balance_cmp']

                    group_vals['balance'] += acc_vals['balance']
                    account_lines.append(acc_vals)

                # 3. Agregar grupo al reporte
                lines.append(group_vals)
                # 4. Agregar cuentas del grupo
                lines += account_lines
